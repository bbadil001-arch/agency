      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-10 sm:gap-14">
        <div className="flex flex-col justify-between gap-8 md:flex-row md:items-center">
          ...
          <nav className="flex flex-wrap gap-x-6 gap-y-3 text-sm text-foreground/50" aria-label="Footer navigation">
            {links.map((link) => <a key={link.key} href={link.href} className="transition hover:text-foreground">{t(link.key)}</a>)}
            <a href="/privacy-policy" className="transition hover:text-foreground">{t('privacy')}</a>
            <a href="/terms" className="transition hover:text-foreground">{t('terms')}</a>
          </nav>
        ...
        <div className="flex flex-col justify-between gap-3 border-t border-white/[0.06] pt-6 text-xs text-foreground/35 sm:flex-row">
          <p>© 2026 AGENCY. Built in Morocco, made for anywhere.</p>
