...
export function Blog({ limit, routeLocale }: { limit?: number; routeLocale?: Locale }) {
  const { copy, locale } = useLocale();
  const pathname = usePathname();
  const pathLocale = pathname.match(/^\/(en|fr|ar)(?=\/|$)/)?.[1] as Locale | undefined;
  const resolvedLocale = routeLocale ?? pathLocale ?? locale;
  const localizedPosts = getLocalizedBlogPosts(resolvedLocale);
  const posts = limit ? localizedPosts.slice(-limit) : localizedPosts;
  const blogPrefix = routeLocale || pathLocale ? `/${resolvedLocale}` : '';
